        ...
        for (line in File(inputName).readLines()) {
            ...
            for (letter in 1 until line.length) {
                if (Regex("""[ЖЧШЩ]""").matches(line[letter - 1].toString()) or
                    ...
                    when {
                        (line[letter]).toString() == "Ы" -> it.write("И")
                        (line[letter]).toString() == "ы" -> it.write("и")
                        (line[letter]).toString() == "Я" -> it.write("А")
                        (line[letter]).toString() == "я" -> it.write("а")
                        (line[letter]).toString() == "Ю" -> it.write("У")
                        (line[letter]).toString() == "ю" -> it.write("у")
                        else -> it.write(line[letter].toString())
                    }
                } else
...
fun markdownToHtmlSimple(inputName: String, outputName: String) {
    ...
    File(outputName).bufferedWriter().use {
        ...
        //если пустой, то вывод сразу
        var check = f.lastIndex
        while (f[check].isEmpty()) {
            f -= f[check]
            check = f.lastIndex
        }
        //смотрю с конца, если последний пустой - удаляю
